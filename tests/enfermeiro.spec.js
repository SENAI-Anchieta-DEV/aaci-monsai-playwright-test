...
// ─── Suite: Enfermeiro ────────────────────────────────────────────────────────
test.describe('Fluxo: Enfermeiro (Criar / Atualizar / Deletar)', () => {

  // ── CRIAR ──────────────────────────────────────────────────────────────────
  test.describe('Criar Enfermeiro', () => {
    ...
    test('Deve cadastrar um enfermeiro com dados válidos', async ({ page }) => {
      await page.getByLabel('Nome Completo').fill('Ana Enfermeira Teste');
      await page.getByLabel('CPF').fill('741.852.963-00');

      await page.getByLabel('Tipo de Acesso').click();
      await page.getByRole('option', { name: 'ENFERMEIRO' }).click();

      await page.getByLabel('E-mail').fill('enfermeiro.teste@monsai.com');
    ...
    test('Não deve cadastrar com e-mail inválido', async ({ page }) => {
      await page.getByLabel('Nome Completo').fill('Enfermeiro Email Errado');
      await page.getByLabel('CPF').fill('741.852.963-00');
    ...
    test('Não deve cadastrar com senha menor que 6 caracteres', async ({ page }) => {
      await page.getByLabel('Nome Completo').fill('Enfermeiro Senha Fraca');
      await page.getByLabel('CPF').fill('741.852.963-00');

      await page.getByLabel('Tipo de Acesso').click();
      await page.getByRole('option', { name: 'ENFERMEIRO' }).click();

      await page.getByLabel('E-mail').fill('enfermeiro@monsai.com');
      await page.getByLabel('Senha Provisória').fill('abc');
  ...
  test.describe('Atualizar Enfermeiro', () => {
    ...
    test('Deve abrir o modal de redefinição de senha para um enfermeiro', async ({ page }) => {
      const linhaEnfermeiro = page.locator('tr').filter({ hasText: 'ENFERMEIRO' }).first();
      await expect(linhaEnfermeiro).toBeVisible();

      await linhaEnfermeiro.getByRole('button', { name: 'Alterar Senha' }).click();

      await expect(page.getByRole('dialog')).toBeVisible();
      await expect(page.getByText('Redefinir Senha')).toBeVisible();
    });

    test('Deve alterar a senha de um enfermeiro com sucesso', async ({ page }) => {
      const linhaEnfermeiro = page.locator('tr').filter({ hasText: 'ENFERMEIRO' }).first();
      await linhaEnfermeiro.getByRole('button', { name: 'Alterar Senha' }).click();

      await page.getByLabel('Nova Senha').fill('novaSenha789');
    ...
    test('Não deve salvar senha com menos de 6 caracteres', async ({ page }) => {
      ...
      await page.getByLabel('Nova Senha').fill('abc');
      await page.getByRole('button', { name: 'Salvar' }).click();
  ...
  // ── DELETAR ────────────────────────────────────────────────────────────────
  test.describe('Deletar Enfermeiro', () => {
    ...
    test('Deve remover o acesso de um enfermeiro ao confirmar', async ({ page }) => {
      const linhaEnfermeiro = page.locator('tr').filter({ hasText: 'ENFERMEIRO' }).first();
      await expect(linhaEnfermeiro).toBeVisible();

      page.on('dialog', (dialog) => dialog.accept());

      await linhaEnfermeiro.getByRole('button', { name: 'Remover Acesso' }).click();
    ...
    test('Deve cancelar a remoção ao recusar o alerta nativo', async ({ page }) => {
      const totalAntes = await page.locator('tr').filter({ hasText: 'ENFERMEIRO' }).count();
      ...
      const totalDepois = await page.locator('tr').filter({ hasText: 'ENFERMEIRO' }).count();
